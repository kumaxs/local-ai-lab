...
任务队列按 API cursor 以前后页浏览，每页最多 100 项；计数只表示当前页，自动与
...
当前源码的质量适配器会额外生成 `regions.json` 与
`quality_signals.json`。区域记录覆盖图片 OCR、页眉页脚、普通图片、表格、算法、
代码、展示公式和行内数学，并使用 `verified_semantic`、`visual_only`、
`unresolved` 三种明确结果；任何关键区域未解决、记录截断、证据路径不安全或侧车
写入失败都会把任务降为 `degraded_failure`。普通图片只有在实际裁剪仍存在时才可
标记为非关键 `visual_only`。

结构区域必须同时绑定最终文档节点、标准化正文身份、源 PDF 摘要、页码/bbox 和
类型正确的源图；缺失、重复、额外或互相冲突的声明都会失败。跨页算法只有在每一页
均有真实源证据且 HTML/Markdown 均已绑定时才可交付；当前产物 schema 无法满足时
会明确 `unresolved`，不会拿第一页裁剪冒充完整算法。算法 manifest 还必须与
`algorithm_blocks.json` 的全部贡献节点、节点类型、正文 hash 和联合 bbox 完全一致。

表格门禁不再只相信产物生成器的覆盖标志，还会独立检查单元格行号与 bbox：单行
单元格跨过另一语义行中心、多个高单元格把多行数值压成一行、声明网格存在未覆盖
槽位、或空网格没有显式 fallback 时都会失败。行内数学绑定会保留运算符、关系符、
标点和 Unicode 数学符号，且不接受截短候选。算法盘点也支持严格的
Definition/Procedure 标题与跨页编号步骤；图片区域内的 OCR 不会再通过算法分组
旁路回流正文。算法若以表格网格作为语义贡献者，还必须独立证明网格非空、完整、
无重叠且 offset/span 一致，普通空表 fallback 不会放宽算法门禁。没有来源结构、
页码/bbox 或图片关系支撑的纯字符串“坐标轴尾部”猜测不会删除正文或长图注。规则
不读取论文名、样本名或已知哈希。
