...
async function loadConfig() {
  try {
    UI_STATE.config = await requestJson("/v1/system/config");
    renderConfigRows();
...
async function refreshDashboardData() {
  ...
  try {
    const jobsUrl = stateFilter.value
      ? `/v1/jobs?state=${encodeURIComponent(stateFilter.value)}&limit=100`
      : "/v1/jobs?limit=100";
    ...
    if (refreshSequence !== UI_STATE.refreshSequence) {
      return;
    }
    UI_STATE.lastHealthFetch = capabilities;
    UI_STATE.lastStorageFetch = storage;
    UI_STATE.jobs = Array.isArray(jobsResponse.items) ? jobsResponse.items : [];
    UI_STATE.jobs.forEach((item) => {
      // Pair each newly returned server timestamp with the time this response
      // reached the browser. Reusing an older receipt time would make TTL
      // countdowns drift forward on every poll.
      item.__receivedAt = Date.now();
    });
    renderSystemCards();
    renderStorageCards();
    renderJobs();
