#!/usr/bin/env python3
"""Minimal Docling Serve quality parity adapter probe.

This script demonstrates the request and post-processing shape needed to keep
Docling Serve close to the prior services/docling-service quality-first policy.
It is intentionally small and writes only to a caller-provided output directory,
which should be a temporary path during validation.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--force-ocr-on-gxx", action="store_true")
    parser.add_argument("--enable-formula-mlx", action="store_true")
    parser.add_argument(
        "--image-export-mode",
        choices=["embedded", "referenced", "placeholder"],
        default="embedded",
        help="Use embedded for validation to avoid stray referenced image files.",
    )
...
def base_options(args: argparse.Namespace, *, force_ocr: bool) -> dict[str, Any]:
    options: dict[str, Any] = {
        ...
        "do_formula_enrichment": bool(args.enable_formula_mlx),
...
def summarize_response(
    ...
    metadata = {
        ...
        "conversion_policy": "quality_parity_probe",
        "sample_name": name,
...
def run_conversion(
    ...
    *,
    ...
    response = post_json(f"{args.serve_url.rstrip('/')}/v1/convert/source", payload)
    ...
    metadata, status = summarize_response(name, response, wall_time, options, warnings)
    return response, metadata, status


def main() -> int:
    args = parse_args()
    name = args.sample_name or args.input_file.stem
    ...
    gaps = [
        ...
        "This adapter records quality signals but is not a product decision to keep a wrapper layer.",
    ]
